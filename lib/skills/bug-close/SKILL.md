...
Finalize a bug that has been fixed. Records the root cause, the resolution (commit, PR, regression test), and moves the file to `wiki/work/bugs/  `. Refuses to close without a regression test — silent closes are the worst-case anti-pattern.
...
| Linked task | `wiki/work/tasks/NNN-slug.md` path or `—` | No |
...
| Outcome | Move |
|---------|------|
| Verify and close | `git mv wiki/work/bugs/  NNNN-slug.md wiki/work/bugs/  ` |
| Mark fixed | No move |
| Trash outcomes | `git rm wiki/work/bugs/  NNNN-slug.md` (fall back to `rm` if `git rm` fails) |

Fall back to `mv` if `git mv` fails. Update `Status:` and `Last updated:` in the file via `Edit`.

### Step 8: Update the Bug Index

In `wiki/work/bugs/README.md`, update the row for this bug:
...
- For `verified`: a one-line clustering check — search `wiki/work/bugs/  ` for the last 3 closes with overlapping `Tags` and surface them as "related recent closes" so the user can spot systemic issues.
