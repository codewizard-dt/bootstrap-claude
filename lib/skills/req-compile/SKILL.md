...
name: req-compile
description: >-
  Compile a requirement into a running, production-grade, delightful system with minimal human time —
  by routing every claim to its cheapest re-runnable check and building the dominant oracle
  first. Use this whenever a project has a wiki/work/requirements/ REQ file to build or execute
  ("build this per the requirement", "execute the spec", "turn these requirements into a working
...
Turn a requirement into a working system that **self-verifies**, surfacing the human only where no
...
## Locating the requirement

Parse `$ARGUMENTS` to find the requirement file:
...
1. **Compile the requirement into claims.** Turn every must-have — and every graded bonus — into an
   explicit claim, each tagged with the oracle that proves it. This list *is* the done-criterion;
   you return to it in step 8. The requirement's User Stories and Acceptance Criteria are the
   primary source; Success Metrics generate the eval claims.
...
8. **Audit against the requirement, then bind the gate.** Re-grade the running system against the claim
...
- **Right-size to stakes; no CI.** Build the smallest correct slice for the stated stakes; note
  ...
  don't add CI/CD unless the requirement demands it.
...
4. **Deploy** — is a live deployment in scope (a hard oracle), or is a productionization
   description sufficient? (Often answerable from the requirement — confirm, don't assume.)
