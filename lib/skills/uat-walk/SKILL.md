...
1. **If a file path is provided** (e.g., `wiki/work/uat/UAT-003-user-auth.md`):
   - Use the path directly

2. **If a number-slug is provided** (e.g., `UAT-003` or `003-user-auth`):
   - Search `wiki/work/uat/` for a matching file (files never move — no completed/ subdir)
   - If not found, STOP and report the error

3. **If only a description or number is provided** (e.g., `user auth` or `3`):
   - Use Serena list_dir `wiki/work/uat/` for a matching UAT file
...
- If the file is not in `wiki/work/uat/`, warn the user and confirm they want to proceed
...
- **All complete** (no `[FAIL]` or `[FIXING]` markers remain — every test is `[x] Pass` or `[SKIP: ...]`):
  1. **Flip UAT status:** Edit `status: pending` (or `in-progress`) → `status: passed`; bump `updated:` to today in the UAT file's frontmatter.
  2. **Flip task status:** Read the UAT file's `task:` frontmatter to get the task ID (e.g. `TASK-014`). Open `wiki/work/tasks/TASK-NNN-slug.md`, edit `status: in-progress` → `status: done`; bump `updated:`.
