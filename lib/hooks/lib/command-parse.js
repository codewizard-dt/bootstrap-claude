...
// `defer` hands the decision back to Claude Code's own native permission
// handling; the contract does not document permissionDecisionReason as
// meaningless there, so — same as `allow`/`deny` — we keep passing it
// through rather than risk silently dropping useful context.
