...
// the command line") and env-file-guard.js:81 says so in its own deny message.
//
...
//
// Fails: open — a payload shape this hook does not expect falls through to allow
// (the try/catch wrapping the dispatch at the bottom of this file, and
// lib/command-parse.js's own handler catch). Ambiguous MATCHES go the other way
// and block, which is why a segment that merely quotes a blocked form fires.

const path = require('path');
const { readHookInput, splitSegments, tokenize, deny } = require('./lib/command-parse');

// KEPT BYTE-IDENTICAL WITH env-file-guard.js:48-55. If the allow-list changes,
...
//
// BUG-0007 tracks extracting this into lib/ so the two copies cannot drift. The
// twin now carries the same warning (env-file-guard.js:36-47), so both sides of
// the duplication point at each other and at the bug; README.md:522-525 records
// why the extraction was deferred.
