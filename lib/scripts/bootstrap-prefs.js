...
//
// --project <dir> resolves to <dir>'s git repo root (walking up for `.git`),
// not <dir> itself — a caller running from a subdirectory still hits the
// one project-scoped file (see findProjectRoot). Non-git dirs fall back to
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === '--get') {
      ...
      opts.projectDir = findProjectRoot(argv[++i]);
    } else if (arg === '--target') {
      opts.target = argv[++i];
    } else if (arg === '--schema') {
      opts.schema = argv[++i];
...
// Resolves to the repo root, not the caller's cwd — a Bash command run from
// a subdirectory would otherwise miss the project's real prefs file. `.git`
// may be a file (worktrees) or a directory; falls back to the literal path
// (absolute) when none is found, leaving non-git test scratch dirs unaffected.
