...
All five `consumer: skill` keys name `install-global.sh` as their `askedBy`,
...
the one site that writes the answer. For all five keys that site is
...
| `gitignore.infoExclude` | project | installer | `true \| false` | `unset` | `merge-gitignore.sh` | Keep `.serena/`, `raw/`, and `wiki/` out of git on this machine via `.git/info/exclude` |
