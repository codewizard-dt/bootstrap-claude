...
The project is a **template repository** with a thin CLI wrapper for npm distribution. The `.claude/skills/` directory holds 27 Skills directory entries that Claude Code loads as custom instructions — each skill is a self-contained workflow spec with mandatory MCP tool requirements, step-by-step logic, and integration with Serena's memory system. The `.docs/` directory provides four parallel artifact lifecycles: PRDs (`.docs/prd/` — lean Product Requirements Documents; `draft → approved → archived/superseded/trashed`), tasks (`active → completed → trashed`), UAT (`pending → completed/skipped/trashed`), and ADRs (`.docs/adr/` — append-only Architecture Decision Log with per-decision supersession). Four Bash scripts handle the lifecycle: `.scripts/setup-project.sh` (new project bootstrap), `.scripts/update-project.sh` (incremental template sync), `.scripts/setup-deployment.sh` (CI/CD scaffolding seam — copies `.github/` workflows and `.gitleaks.toml`, called once on setup, never on update so hand-customized workflows are preserved), and `.scripts/bootstrap-serena.sh` (headless Serena project.yml init via `claude --print`). `bin/cli.js` wraps these as `setup`, `update`, `install`, and `deploy` for `npx` invocation.
...
- **Template synchronization** — run `npx bootstrap-claude update` on existing projects to pull in the latest skill and `.docs/` scaffold improvements without overwriting project-specific files. Run `npx bootstrap-claude deploy` to opt an existing project into CI/CD by scaffolding `.github/` workflows (Gitleaks secret scanning + a generic Docker build/push/deploy template with a self-disabling guard when no Dockerfile is present).
