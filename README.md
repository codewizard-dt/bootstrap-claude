...
It is designed for developers who treat AI agents as first-class collaborators. The slash commands, task file format, and UAT system are all engineered to give Claude (and other agents) clear, unambiguous instructions — enabling reliable delegation of planning, implementation, testing, and documentation tasks. The full workflow spans requirements capture (`/create-prd`), architectural decisions (`/create-adr`), task creation (`/add-task`), implementation delegation (`/tackle`, `/now`), test generation (`/uat-generator`), interactive walkthrough (`/uat-walkthrough`), and documentation updates (`/update-docs`).
