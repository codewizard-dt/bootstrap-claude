...
**Direct implication for uses::[[serena]]**: Serena is registered as a **stdio** server (`uvx ... serena start-mcp-server`) at **local** scope. Neither of those facts affects process count — as a stdio server, it spawns one fresh `uvx`-launched process per Claude Code session regardless of scope. Three windows/sessions open against the same project means three independent Serena processes; there is no daemon-sharing mechanism available to it today (and none is on a committed roadmap). If constant N-process overhead ever becomes a real problem for Serena specifically, the only documented fix would be switching it to an HTTP-mode registration (if Serena ever ships one) backed by a shared long-lived process — the same pattern already used for brave-search/Playwright — not a scope change.
