...
> **Contradiction:** derived_from::[[mcp-scope-performance-behavior]] (2026-08-15) found that Claude Code's official docs *do* document a variable-expansion mechanism for `.mcp.json` (and local/user `~/.claude.json`) entries — `${VAR}`/`${VAR:-default}` expansion, including a `CLAUDE_PROJECT_DIR` variable injected into the spawned server's own environment (requires a fallback default like `${CLAUDE_PROJECT_DIR:-.}` since it isn't set in Claude Code's own environment). This directly conflicts with this page's "no variable-expansion mechanism ... was found" claim. It is unclear whether this was newly documented after 2026-08-14 or simply missed in this page's original research pass. **Not yet independently verified** against Serena's actual `--project` flag — until that test is run, the "keep Serena at local scope" recommendation below stands unchanged, but the "no portability mechanism exists" framing should not be restated as settled fact.
