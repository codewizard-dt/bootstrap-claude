...
**Corroborated from the outside, 2026-09-13.** An ecosystem survey (derived_from::[[claude-code-ecosystem-2026]]) independently reached the same conclusion by reading only this repo's own `lib/hooks/README.md` and `lib/scripts/README.md`, which state the "guardrail, not a boundary" framing four separate times — the recommendation to scaffold `sandbox` config in `install-global.sh`/`setup-project.sh` (as an opt-in step, not a forced default, since `docker` commands are incompatible with the sandbox) remains outstanding.
