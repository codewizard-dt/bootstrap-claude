...
**Known gap in git worktrees with symlinked `wiki/`/`raw/`**: the installed script's `.git/info/exclude` check assumes `.git` is a directory, which fails silently inside a worktree (where `.git` is a `gitdir:` pointer file), and its re-inclusion `rg` calls omit `--follow`, so symlinked directories are skipped even when the sentinel lookup succeeds. See relates_to::[[file-suggestion-worktree-symlink-gap]].
