...
- [BUG-0009 — `--set` reports success writing a key into a layer that never reads it](BUG-0009-set-writes-scope-inert-key-silently.md) — `bootstrap-prefs.js --set` never calls `scopePermitsLayer`, so a `global`-scope key writes into a project file, exits 0, and reads back `unset` at both `--get` and `--list` · open · —
- [BUG-0010 — install-global.sh output overstates what happened](BUG-0010-install-global-output-overstates-what-happened.md) — two message-only defects: the `ask` confirmation denies its own write, and the completion summary claims `MCPs` on every `--skip-mcps` run (i.e. every setup and update) · open · —
