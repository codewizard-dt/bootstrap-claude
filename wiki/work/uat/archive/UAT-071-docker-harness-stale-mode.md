...
### UAT-EDGE-004: A real `run.sh stale` run seeds `c33808d`'s state, tolerates its expected seed-step failure, and the current `update-project.sh` now prints its own explicit PASS/FAIL classification line for its real migration work against that seeded state
- **Scenario**: TASK-071's own "Step 2 manual verification (2026-08-22)" Notes subsection records a real, live run of `./test/docker/fresh-machine/run.sh stale` against the cached `bootstrap-claude-fresh-machine` image: the old-release seed stage runs to completion through all its real work and fails only at the expected last step (Serena `project.yml` bootstrap), then the current `update-project.sh` stage runs its own real migration work (skill-set migration, deny-list/hooks-wiring/CLAUDE.md/aliases checks, wiki scaffold sync, MCP-tools guide rebuild) against that seeded state. TASK-077 has since given that CURRENT-checkout `update-project.sh` call its own independent classification (unchanged: the OLD checkout's tolerant `setup-project.sh` seed call): `run.sh stale` now prints `stale: PASS — update-project.sh completed successfully` (full success) or `stale: PASS — update-project.sh reached the expected, documented Serena-bootstrap failure point and nothing earlier` (the known, expected failure), exiting 0 in both cases; only a genuinely different failure produces `stale: FAIL — ...` and exit 1. Since `update-project.sh` currently fails only at the documented Serena-bootstrap step in this environment (no known regression), `run.sh stale` is expected to print the "reached the expected, documented..." PASS variant today and exit 0.
- **Steps**:
  ...
  4. Confirm the current checkout's `update-project.sh` stage then runs afterward against the same seeded scratch dir, performing its own real migration work, before printing a `stale: PASS — ...` (or `FAIL`) line.
- **Command**:
  ```bash
  ./test/docker/fresh-machine/run.sh stale; echo "exit: $?"
  ```
- **Expected Result**: The old-release seed stage runs its real work and fails only at the Serena `project.yml` bootstrap step (never earlier, never for a missing foundational dependency or a `git archive`/extraction error). The current checkout's `update-project.sh` stage then runs its own real migration work and prints `stale: PASS — update-project.sh completed successfully` or `stale: PASS — update-project.sh reached the expected, documented Serena-bootstrap failure point and nothing earlier`, exiting 0. In this environment, `update-project.sh` is expected to fail only at the documented Serena-bootstrap step, so `run.sh stale` should print the "reached the expected, documented..." variant and exit 0; only a genuinely different failure would produce a `stale: FAIL — ...` line and exit 1.
- **Repeatable Unit Test**: Not applicable: requires a live Docker daemon, a `git archive` of real repo history, and two full non-interactive `setup-project.sh`/`update-project.sh` runs per invocation, but the command now emits a single, deterministic `stale: PASS`/`FAIL` classification line — reclassified from Manual to a live-command case (2026-09-02 fix) since UAT-CORE's channel 1 (extractable `**Command**:` + machine-checkable Expected) needs no unit-test harness to auto-judge this, now that `run.sh stale` emits an explicit status line instead of a bare, ambiguous exit code.
- [x] Pass <!-- 2026-09-03 -->
...
- **Automation reassessment (2026-08-27) — resolved (2026-09-02)**: UAT-EDGE-004 stayed Manual because `run.sh stale` exited with `update-project.sh`'s own exit code, which was then *always* non-zero due to the documented, expected Serena-bootstrap limitation (same root cause as UAT-060's EDGE-011) — a bare exit-code check couldn't distinguish that expected failure from a genuine regression, since both produced the identical exit code. Unlike `run.sh idempotency` (TASK-072), `stale` had no explicit `stale: PASS`/`stale: FAIL` output line to key off. TASK-077 added one, mirroring `idempotency` mode's own pattern (and the same fix applied to `setup`/`update` in UAT-060's Gaps): `run.sh stale` now prints `stale: PASS`/`FAIL` for the current checkout's `update-project.sh` call, so both PASS variants (full success vs. the expected, documented Serena-bootstrap stop) are distinguishable from a genuine `FAIL` regression. UAT-EDGE-004 above has been promoted to a live-command case accordingly.
