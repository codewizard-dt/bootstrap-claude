...
4. If found outside `open/`, STOP and tell the user the triage state. A bug in `in-progress/`, `closed/`, or `trashed/` cannot be re-triaged via this skill — they re-open it manually (move file back to `open/` and flip `Status: new`) or use `/close-bug` if appropriate.
...
Before triaging, every required-on-report field from `/file-bug` must be present and non-templated. Re-check:
...
- If the user chose **Start work now**: ask whether to spawn a fix task via `/add-task`. If yes, suggest the command — do not invoke it automatically.
...
- Index row updated
- Suggested next step:
  - `Stay triaged` / `Start work now` → `/close-bug BUG-NNNN` when the fix is merged and verified
  - Trash outcomes → terminal; to undo, `git mv` the file back and re-run `/triage-bug`
