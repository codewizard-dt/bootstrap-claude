...
Tasks remain in `.docs/tasks/active/` until all UAT tests pass, at which point this command moves them to `.docs/tasks/completed/`. UAT files are generated by `/uat-generate` and placed in `.docs/uat/pending/`.
...
- **Some failed** (any `[FAIL]` or `[FIXING]` markers remain): Keep file in `pending/`. Keep screenshots (useful for debugging). Suggest next steps:
  ```
  To fix failures and re-test:  /uat-walk .docs/uat/pending/<file>.uat.md
