...
Once a PRD is approved, it is **immutable in substance**. Future scope changes go through `/prd-update`, which appends an `## Amendment N` block rather than rewriting prior content. This command is the one-way gate into that immutable state — get the audit right.
...
3. **Verify the status is `draft`**:
   ...
   | `approved` | Stop. Tell the user the PRD is already approved and immutable in substance; suggest `/prd-update <file> <change>` for any amendment |
...
| No — they have not | **Stop.** Leave `Status: draft` in place. Report what's blocking and suggest the user circulate the draft (link to the file) before re-running `/prd-finalize`. |
...
If this PRD's row does not yet exist in the README's Index table (e.g. the placeholder "_No PRDs yet_" row is still present, or `/prd-create` did not insert one), insert a new row using the column format documented in the README:
...
| `Linked ADRs` | `—` (filled later by `/prd-extract-decisions`) |
| `Linked Tasks` | `—` (filled later by `/task-add --prd`) |
...
|------------|----------------|
| `/prd-extract-decisions <file>` | The default next step — extract Architecturally Significant Requirements and propose ADR candidates |
| `/task-add --prd PRD-NNN <description>` | Only when the PRD's scope is small enough to skip ADRs entirely (no ASRs surface) |
| `/prd-update <file> <change>` | If a scope change surfaces post-approval — appends an `## Amendment N` block; never rewrites |
...
3. **Never alter approved PRDs** — refuse to run on a target whose status is `approved`, `archived`, `superseded`, or `trashed`; suggest `/prd-update` for amendments to approved PRDs
...
4. **Never auto-create downstream artifacts** — `/prd-extract-decisions`, `/task-add`, ADRs, and tasks are user-invoked. This command suggests; the user runs
