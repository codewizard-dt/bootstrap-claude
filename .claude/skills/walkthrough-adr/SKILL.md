...
Systematically walk through every decision block in an ADR file. For each decision, present a compact summary of drivers, options, the recommended choice, and trade-offs, then ask the user via `AskUserQuestion` to **confirm**, **change**, **defer**, or **skip**. Sibling decisions are treated independently; the file's other blocks are never touched while one block is being discussed.

This skill is a **review pass**, not a finalizer. It does **not** flip status to `accepted` — that responsibility belongs to `/finalize-adr <file>#<DM>`, which runs the full E-C-A-D-R audit and supersession check. Use this command when you want a low-friction walkthrough to ratify (or revise) the architecture choices in a draft ADR before sending each decision through `/finalize-adr`.

**Read `<adr-dir>/README.md` first.** It defines the Decision Group model and the per-decision rules this command must respect (especially: siblings are inviolate; per-decision metadata is mandatory; tables not bullets; mermaid for flows).
...
`/create-adr` → **`/walkthrough-adr`** → `/finalize-adr <file>#<DM>` (per decision) → `/add-task` → `/tackle`

The walkthrough is optional — `/finalize-adr` alone is sufficient when only one decision needs ratifying. Use `/walkthrough-adr` when an ADR file has 2+ proposed decisions and you want a single guided pass to align on every choice before finalizing each.
...
|------------|------------|
| `<path>` (e.g. `.docs/adr/0007-session.md`) | Use as-is |
| `<NNNN-slug>` (e.g. `0007-session`) | Search ADR dir for `NNNN-slug.md` |
| `<NNNN>` (e.g. `7` or `0007`) | Pad to 4 digits; `mcp__serena__find_file` with mask `NNNN-*.md` |
| Empty / missing | List every ADR file with at least one `proposed` decision; ask the user via `AskUserQuestion` to pick |
...
4. **Classify per status:**
   ...
   | `superseded by ADR-X#DY` | **Informational only** — print summary with the supersession target; skip Q&A |
...
1. **Present the decision summary** as tables (never bullets):
   ...
   Cap the summary at ~25 lines. If a section is missing or all-placeholder, mark it `⚠️ gap — surface at finalize time`. **Do not edit gaps here** — that is `/finalize-adr`'s job.
...
3. **Apply edits only when the verdict is Confirm-without-bold or Change.** Use `Edit` (never `sed`/`echo`). Constrain `old_string` to text unique within the target block to avoid touching siblings.
   ...
   | Any verdict | **Never** flip `Status` to `accepted` (that is `/finalize-adr`'s gate); **never** edit sibling decision blocks; **never** edit the index or relationship graph |
...
Anything beyond these three fields, or any change requiring research (drivers, consequences, validation thresholds), is **out of scope** — defer to `/finalize-adr` and report it in Step 5.
...
| D2 | Session lifetime | proposed | Confirmed | bolded chosen option | `/finalize-adr 0007-session#D2` |
| D3 | Invalidation trigger | proposed | Changed (Webhook → Polling) | rewrote outcome | `/finalize-adr 0007-session#D3` |

Append:

- **Confirmed proposed decisions** — list `/finalize-adr <file>#<DM>` commands the user can run next (one per confirmed decision).
- **Deferred decisions** — list IDs the user chose to revisit; suggest re-running `/walkthrough-adr` later.
- **Gaps surfaced but not fixed** (placeholders, missing mermaid, asymmetric tables, etc.) — flag explicitly so the user knows `/finalize-adr` will pick them up.
...
5. **Never invent content** — if the user picks a new option that lacks a comparison row, do **not** synthesize one; flag it and recommend `/finalize-adr` (which will demand the research).

---

## CRITICAL Rules

1. **No status flips** — this command never writes `Status: accepted`, `Status: deprecated`, or `Status: superseded by …`. Those transitions belong to `/finalize-adr` (and to the supersession path `/finalize-adr` enforces).
...
3. **No supersession work** — supersession detection and the two-block cross-reference rule are `/finalize-adr`'s job. Do not search other ADR files; do not edit the index or relationship graph.
4. **Refuse on missing-`Tags` write requests** — if the user asks to confirm a decision whose `Tags` line is empty and they decline to fill it in Step 4, leave the block as-is and tell them `/finalize-adr` will block on it.
