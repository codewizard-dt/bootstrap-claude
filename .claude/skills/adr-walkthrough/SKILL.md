...
This skill is a **review pass**, not a finalizer. It does **not** flip status to `accepted` — that responsibility belongs to `/adr-finalize <file>#<DM>`, which runs the full E-C-A-D-R audit and supersession check. Use this command when you want a low-friction walkthrough to ratify (or revise) the architecture choices in a draft ADR before sending each decision through `/adr-finalize`.
...
`/adr-create` → **`/adr-walkthrough`** → `/adr-finalize <file>#<DM>` (per decision) → `/task-add` → `/tackle`

The walkthrough is optional — `/adr-finalize` alone is sufficient when only one decision needs ratifying. Use `/adr-walkthrough` when an ADR file has 2+ proposed decisions and you want a single guided pass to align on every choice before finalizing each.
...
1. **Present the decision summary** as tables (never bullets):
   ...
   Cap the summary at ~25 lines. If a section is missing or all-placeholder, mark it `⚠️ gap — surface at finalize time`. **Do not edit gaps here** — that is `/adr-finalize`'s job.
...
3. **Apply edits only when the verdict is Confirm-without-bold or Change.** Use `Edit` (never `sed`/`echo`). Constrain `old_string` to text unique within the target block to avoid touching siblings.
   ...
   | Any verdict | **Never** flip `Status` to `accepted` (that is `/adr-finalize`'s gate); **never** edit sibling decision blocks; **never** edit the index or relationship graph |
...
Anything beyond these three fields, or any change requiring research (drivers, consequences, validation thresholds), is **out of scope** — defer to `/adr-finalize` and report it in Step 5.
...
| D2 | Session lifetime | proposed | Confirmed | bolded chosen option | `/adr-finalize 0007-session#D2` |
| D3 | Invalidation trigger | proposed | Changed (Webhook → Polling) | rewrote outcome | `/adr-finalize 0007-session#D3` |

Append:

- **Confirmed proposed decisions** — list `/adr-finalize <file>#<DM>` commands the user can run next (one per confirmed decision).
- **Deferred decisions** — list IDs the user chose to revisit; suggest re-running `/adr-walkthrough` later.
- **Gaps surfaced but not fixed** (placeholders, missing mermaid, asymmetric tables, etc.) — flag explicitly so the user knows `/adr-finalize` will pick them up.
...
5. **Never invent content** — if the user picks a new option that lacks a comparison row, do **not** synthesize one; flag it and recommend `/adr-finalize` (which will demand the research).

---

## CRITICAL Rules

1. **No status flips** — this command never writes `Status: accepted`, `Status: deprecated`, or `Status: superseded by …`. Those transitions belong to `/adr-finalize` (and to the supersession path `/adr-finalize` enforces).
...
3. **No supersession work** — supersession detection and the two-block cross-reference rule are `/adr-finalize`'s job. Do not search other ADR files; do not edit the index or relationship graph.
4. **Refuse on missing-`Tags` write requests** — if the user asks to confirm a decision whose `Tags` line is empty and they decline to fill it in Step 4, leave the block as-is and tell them `/adr-finalize` will block on it.
