...
**Architecture diagrams**: If the project has a meaningful architecture worth showing (layered systems, data flows, service dependencies), include one Mermaid flowchart slide per layer. Keep each diagram small — no more than six or seven nodes. One layer per diagram so each chart stays readable at a glance.
