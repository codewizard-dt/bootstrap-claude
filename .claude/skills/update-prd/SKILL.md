...
| `archived` / `superseded` / `trashed` | Refuse — these PRDs are no longer authoritative; suggest `/create-prd` for new scope |
...
If the **change instructions are empty**, use `AskUserQuestion` to elicit them before proceeding. A `/update-prd` invocation without a described change is malformed.
...
3. **Branch on status:**
   ...
   | `archived` | **STOP.** Tell the user: "PRD-NNN is archived; the product work landed and the PRD is preserved as historical reference. Run `/create-prd` for any new scope." |
   | `superseded` | **STOP.** Tell the user: "PRD-NNN was superseded by `<successor>`. Update the successor PRD instead." |
   | `trashed` | **STOP.** Tell the user: "PRD-NNN is trashed and no longer authoritative. Run `/create-prd` for new scope." |
...
3. If the change involves **adding or removing user stories, success metrics, or non-goals**, surface a one-line reminder: *"This PRD is still `draft`. When you're ready to approve, run `/finalize-prd <path>` to re-audit completeness."*
...
2. **Edit `.docs/prd/README.md`** only if amendment-relevant columns changed:
   ...
   | `Linked ADRs` | No (those tables only update via `/prd-to-decisions`) |
   | `Linked Tasks` | No (those tables only update via `/add-task --prd`) |
...
|----------|------|--------|------------------|
| `ADR-0007#D2` | linked ADR | accepted | Review whether amendment invalidates the decision; if so, write a successor via `/create-adr` and finalize via `/finalize-adr` to supersede |
| `.docs/tasks/active/004-billing-checkout.md` | linked task | WIP | Review whether amendment changes the task scope; if so, run `/update-task .docs/tasks/active/004-billing-checkout.md "<change summary>"` |
...
| Suggested next steps | `/finalize-prd ...` (drafts), `/update-task ...`, `/create-adr ...`, etc. |

If the branch was `direct-edit (draft)` and the change was substantive, include the reminder: *"Run `/finalize-prd <path>` when ready to approve."*
...
6. **Refuse on `archived` / `superseded` / `trashed` status** — these PRDs are no longer authoritative; suggest `/create-prd` for new scope
...
5. **Never auto-update downstream artifacts** — surface linked ADRs and tasks for user review; do not run `/update-task` or `/create-adr` on the user's behalf
