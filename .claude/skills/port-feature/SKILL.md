...
3. **Clarifying questions** (ask before proceeding if ambiguous):
   - Should the port plan produce a task file in `.docs/tasks/`, a report in `.docs/port/`, or both?
...
- Read `.docs/adr/` index (if present) for accepted architectural decisions that constrain the port
...
- Check `.docs/tasks/`, `.docs/prd/`, and `.docs/roadmaps/` for any planned or in-progress work in this area
