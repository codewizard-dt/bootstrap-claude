---
name: prd-to-decisions
description: Extract Architecturally Significant Requirements from an approved PRD, cross-check existing ADRs, and propose Decision Group candidates for /create-adr
...
This skill **never** invokes `/create-adr` itself. It surfaces the commands the user runs next, one per confirmed Decision Group — the same pattern `/finalize-adr` uses to suggest `/add-task`. The user remains in control of which ADRs to write, when, and with what scope.
...
| `draft` | Stop. Tell the user: "This PRD is still in draft. Run `/finalize-prd <path>` first to flip it to `approved`, then re-run `/prd-to-decisions`." |
| `archived` | Stop. Tell the user: "This PRD is closed and the work landed. If a new architectural decision is needed, write a new PRD via `/create-prd`." |
| `superseded` | Stop. Tell the user: "This PRD has been superseded. Run `/prd-to-decisions` against the successor PRD instead." |
...
3. For each candidate ASR, score each ADR decision block using the same Tier-1 tag-overlap signal `/create-adr` and `/finalize-adr` use:
...
4. Build the existing-coverage table:
   ...
   | A3 | ADR-0005#D1 (tags: hris) | proposed | propose new ADR if A3's scope is broader; otherwise extend the proposed decision via the user's next `/create-adr` invocation |
...
| Option | Meaning |
|--------|---------|
| `yes` | Confirm the group; the skill will write a placeholder row in the PRD and the user will run `/create-adr` next |
...
| `skip — non-architectural` | Drop the group; the requirement is shallow enough to handle directly via `/add-task --prd PRD-NNN` |
...
| ASR | ADR | Status |
|-----|-----|--------|
| A1: <verbatim statement> | _to be created via `/create-adr group: search-infrastructure`_ | not yet created |
| A2: <verbatim statement> | [ADR-0003#D2](../../adr/0003-auth-and-access.md#d2-...) | accepted |
| A3: <verbatim statement> | _to be created via `/create-adr hris-integration`_ | not yet created |
...
| `ADR` | If covered by existing ADR: anchor link to the decision block. If creating new: italicized placeholder with the exact `/create-adr` command the user will run |
...
To create the proposed ADRs, run each:
  /create-adr group: search-infrastructure (from PRD-NNN)
  /create-adr hris-integration (from PRD-NNN)

After each /create-adr completes, return to PRD-NNN's `## Linked ADRs` table
...
The following ASRs are best handled directly as tasks (no ADR needed):
  /add-task --prd PRD-NNN "<short description for ASR Ax>"
```

The skill does **not** auto-invoke `/create-adr` or `/add-task`. It surfaces the commands; the user runs them.
...
2. **Never auto-invoke `/create-adr` or `/add-task`.** Surface the exact commands; the user runs them. Same pattern as `/finalize-adr` Step 7.
...
6. **Per-decision tag overlap is the existing-coverage signal.** Match the same Tier-1 logic `/create-adr` and `/finalize-adr` use. Do not invent a different heuristic.
