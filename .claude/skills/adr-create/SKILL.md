...
| Signal | Group into one file | Split (this command writes one decision now; user invokes `/adr-create` again later) |
|--------|--------------------|--------------------------------------------------------------------------------------|
...
2. **If any decision was created with status `accepted` AND supersedes an existing decision** (rare — Step 4 #1 was set to `accepted`), apply the README's two-block cross-reference rule per affected pair. The atomic update covers:
   ...
   **Default to `proposed`.** Supersession at create-time is rare; `/adr-finalize` enforces it atomically when the user is ready.
...
| Suggested next steps | `/adr-finalize <file>#D1`, `/adr-finalize <file>#D2`, … |
