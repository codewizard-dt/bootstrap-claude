---
description: Finalize a single proposed decision (ADR-NNNN#DM) by resolving open questions and flipping its status to accepted
...
# Finalize ADR Decision

Take a **single proposed decision block** within an ADR file, surface every open question or unresolved placeholder, ratify the decision with the user, **enforce the supersession rule** against any prior accepted decision in the same area, then flip *that decision's* status to **accepted**.

Each decision is finalized **independently**. A file with three proposed decisions is finalized via three separate `/finalize-adr` invocations. Sibling decisions in the same file are never touched.

Once a decision is accepted, it becomes immutable — to change a past decision, write a new one (in any ADR file) via `/create-adr` and finalize it via `/finalize-adr` to supersede the old one.

**Read `<adr-dir>/README.md` first.** It defines the Decision Group model, the per-decision supersession rule, the two-block cross-reference rule, and the relationship graph that finalization must keep in sync.
...
|------------|------------|
| `<path>#DM` (e.g. `.docs/adr/0007-session.md#D2`) | Use file as-is; decision = `D2` |
| `<NNNN-slug>#DM` (e.g. `0007-session#D2`) | Search ADR dir for `NNNN-slug.md`; decision = `D2` |
...
| Empty / missing | List every `proposed` decision across all ADR files (one row per `ADR-NNNN#DM`) and ask the user to pick |
...
3. **Verify the decision's status is `proposed`**:
   ...
   | `accepted` | Stop. Tell the user this decision is already accepted and immutable; suggest `/create-adr` to write a successor that supersedes it |
...
5. **Build the Audit Report** in your working context:
   ...
   ```
   File: 0007-session-management.md
...
The README's supersession rule operates **per decision**, not per file. The check searches across **every ADR file** for accepted decision blocks sharing this decision's tags.

**Detection procedure:**

1. Use `mcp__serena__list_dir` to enumerate all ADR files
...
|-----------|--------|--------------|---------------|--------------|---------|
| ADR-0003#D2 | accepted | yes (`session-lifetime`) | yes | n/a | likely supersede |
| ADR-0005#D1 | accepted | no | no | no | unrelated |
...
| Candidate already `superseded by ADR-X#DY` | Walk the chain to ADR-X#DY; that's the area's current head — supersede it instead |
...
5. **Update the decision's metadata block:**
   ...
   | `Supersedes` | Confirmed `ADR-MMMM#DK` or `none` |

6. **If Step 2.5 identified a supersession target**, apply the README's two-block cross-reference rule **atomically**. Both blocks must change or neither does. The supersession target may be in the same file or a different file:

   | Site | Required Edit |
   |------|---------------|
   | This decision (the superseder) | `Supersedes: ADR-MMMM#DK` in metadata; `### Links` lists `Supersedes [ADR-MMMM#DK](MMMM-slug.md#dK-...)` |
   | Superseded decision block (in `MMMM-slug.md`, possibly the same file) | Flip `Status: accepted` → `Status: superseded by ADR-NNNN#DX`; insert callout `> **Superseded by [ADR-NNNN#DX](NNNN-slug.md#dX-...)** on YYYY-MM-DD` directly **under the superseded decision's H2** (NOT the file's H1) |
...
1. **Index — target row** (the decision being finalized):
   ...
   | `Supersedes` | `[ADR-MMMM#DK](MMMM-slug.md#dK-...)` if Step 2.5 named one, else `—` |
...
2. **Index — superseded row** (if applicable):
   ...
   | Column | New Value |
   |--------|-----------|
   | `Status` | `superseded by ADR-NNNN#DX` |
   | `Superseded By` | `[ADR-NNNN#DX](NNNN-slug.md#dX-...)` |

3. **Relationship Graph** (the mermaid `flowchart` block):
   - Add a node for the newly accepted decision (`ADR-NNNN#DM`) with its title and date
   ...
   - Subgraphs may group nodes per ADR file or per decision area — choose what reads best

If the README does not exist, create it from the template structure in `<adr-dir>/README.md` documented under `/create-adr` Step 6.

**Verify all three sections agree** before saving — index says `superseded by ADR-7#D1` while the graph still shows the old node as `accepted` is a broken commit.

### Step 7: Cross-link follow-up work

Scan the finalized decision's `### Consequences` for `🔁 Follow-up` rows. For each:

| Follow-up Type | Action |
|----------------|--------|
| Implementation work ("write task: ...") | Suggest `/add-task <description referencing ADR-NNNN#DM>` |
| Time-based revisit ("revisit in 6 months") | Offer to `/schedule` a background agent for the revisit date |
| Metric to instrument | Suggest `/add-task` or fold into existing task — ask the user |

Do **not** auto-create tasks or schedules — present them as suggestions.

### Step 8: Update memory if the ratified decision establishes a new pattern
...
| Index updated | yes |
| Graph updated | yes |
| Suggested next steps | `/add-task ...`, `/schedule ...`, `/finalize-adr <file>#Dother`, etc. |
...
3. **Never alter accepted decisions** — refuse to run on an already-accepted target; suggest `/create-adr` to write a successor
