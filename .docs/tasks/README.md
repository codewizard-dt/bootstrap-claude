...
>
> - `/add-task` → appends a row to **Active**
> - `/tackle` → updates `Progress` and `Flags` after each cycle
> - `/uat`, `/uat-skip` → moves a row from **Active** to **Completed**, sets `UAT`
> - `/trash-task` → removes a row entirely (or moves it under a Trashed section if one exists)
>
