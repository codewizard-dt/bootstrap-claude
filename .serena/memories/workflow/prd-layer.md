...
```
/create-prd → /finalize-prd → /prd-to-decisions → /create-adr → /finalize-adr → /add-task → /tackle → /uat-*
...
| Skill | Purpose |
|-------|---------|
| `/create-prd` | Socratic Q&A elicitation → `.docs/prd/active/NNN-slug.md` (status: draft) |
| `/finalize-prd` | Completeness audit + stakeholder gate → flip to approved |
| `/prd-to-decisions` | Extract Architecturally Significant Requirements (ASRs); propose Decision Groups; surface `/create-adr` commands; bidirectional cross-link |
| `/update-prd` | Append-only `## Amendment N` for approved PRDs; direct edits for drafts; surface downstream ADR/task impact |
| `/trash-prd` | Move to `.docs/prd/trashed/`; never auto-cascades to linked ADRs/tasks |
...
|-------|------------------------------|
| PRD draft | Mutable (direct edits via `/update-prd`) |
...
| Direction | Mechanism |
|-----------|-----------|
| PRD → ADRs | `/prd-to-decisions` populates the PRD's `## Linked ADRs` table |
| ADR → PRD | `/prd-to-decisions` adds `Source PRD: PRD-NNN` to each ADR's `### Links` section |
| PRD → tasks | `/add-task --prd PRD-NNN` populates the PRD's `## Linked Tasks` table |
| Task → PRD | `/add-task --prd` adds `**PRD**: PRD-NNN ([file](...))` to the task |
...
- **PRD-as-Spec** — implementation details smuggled into PRD body (`/finalize-prd` rejects)
...
- **Amendment Avoidance** — rewriting approved PRDs instead of using `/update-prd`
- **Phantom Linkage** — only `/prd-to-decisions` and `/add-task --prd` populate the linkage tables

## When to skip the PRD layer

For small/internal work, jump directly to `/add-task` (or `/create-adr` if architecturally significant). The PRD README's "When NOT to Write a PRD" table defines the threshold: bug fixes, refactors, one-line config changes, decisions purely about *how*.
