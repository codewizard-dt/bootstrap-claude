...
- `raw/guides/` — Source guides delivered tier-wise to `wiki/guides/` in target projects (required always-refreshed; optional interactive opt-in).
...
- `lib/scripts/setup-project.sh` — New projects: shared `run_project_sync` (install-global → MCP install → sync-wiki-scaffold → aliases backfill → MCP guide → bootstrap-serena).
- `lib/scripts/update-project.sh` — Existing projects: same shared `run_project_sync` sequence.
