...
| `list_dir` | on | yes | List directory contents, optionally recursive, honouring `.gitignore`. Mandatory in this repo for all directory exploration. Known bug: passing `relative_path="."` can error on some projects (oraios/serena#672) — works fine here. |
| `find_file` | on | yes | Find files by mask in a root path. Mandatory in this repo for filename lookups (overlaps with `Glob` but required by guide). |
| `search_for_pattern` | on | yes | Flexible regex search across the repo with glob filters and context. Required for multi-file/non-symbol searches; `Grep` is an acceptable alternative for content search. |
| `replace_content` | on | yes | Literal or regex text replacement inside a file. `mode` ∈ {literal, regex}, plus `needle`/`repl`. Overlaps with `Edit`; prefer `Edit` for markdown/config. |
| `replace_lines` | *(optional)* | yes | Replace an inclusive line range (0-based) with new content. |
| `insert_at_line` | *(optional)* | yes | Insert content at a specific line number (0-based). |
| `delete_lines` | *(optional)* | yes | Delete an inclusive line range (0-based). |

Policy: symbolic tools first for code edits; file/line tools are available as a fallback when symbolic edits don't fit (e.g. free-form regions inside a function body, non-symbol config blocks).
