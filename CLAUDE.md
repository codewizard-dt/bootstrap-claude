...
- `lib/scripts/setup-project.sh` — Set up a new project: installs hooks/skills/settings globally first (via `install-global.sh --skip-mcps`), then runs the guarded interactive MCP install, syncs wiki scaffold + tiered `wiki/guides/`, backfills `wiki/work/` `aliases:` drift, bootstraps Serena `project.yml`; deployment setup is explicit via `deploy`
