...
- `npx @codewizard-dt/bootstrap setup` — runs `lib/scripts/setup-project.sh` (installs hooks and skills globally, merges the canonical deny list into `~/.claude/settings.json`, syncs wiki scaffold + tiered `wiki/guides/` (required guide always refreshed; optional guides prompted), bootstraps Serena; does NOT set up deployment)
- `npx @codewizard-dt/bootstrap update` — runs `lib/scripts/update-project.sh` (installs hooks and skills globally, merges the canonical deny list into `~/.claude/settings.json`, syncs wiki scaffold + tiered `wiki/guides/`; does NOT touch `.github/` workflows)
- `npx @codewizard-dt/bootstrap install` — runs `lib/scripts/install-global.sh` (installs/updates MCPs, hooks, and skills globally without a project path, and merges the canonical permission deny list from `lib/scripts/templates/settings-deny.json` into `~/.claude/settings.json` — additive-only)
...
- `lib/scripts/install-global.sh` — Installs/updates MCPs, hooks, and skills globally so they are available across all projects, and merges the canonical permission deny list (`lib/scripts/templates/settings-deny.json`, via `lib/scripts/merge-settings-deny.js`) into `~/.claude/settings.json` — `Bash(...)` command patterns plus `Edit(...)`/`Read(...)` file-tool patterns. Also copies `lib/scripts/templates/file-suggestion.sh` to `~/.claude/file-suggestion.sh` and merges the `fileSuggestion` settings key (same script, `--set-key` mode) so `@`-autocomplete sees the `.git/info/exclude`'d wiki dirs again; a pre-existing different `fileSuggestion` is warned about and left alone. Called by both setup and update scripts
- `lib/scripts/setup-project.sh` — Set up a new project: installs MCPs and skills globally, syncs wiki scaffold + tiered `wiki/guides/`, bootstraps Serena `project.yml`; deployment setup is explicit via `deploy`
- `lib/scripts/update-project.sh` — Install MCPs and skills globally and sync the wiki scaffold + tiered `wiki/guides/` into a target project (re-runs `bootstrap-serena.sh` idempotently)
