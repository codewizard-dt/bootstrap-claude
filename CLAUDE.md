...
- `npx @codewizard-dt/bootstrap setup` — runs `lib/scripts/setup-project.sh` (installs hooks and skills globally, merges the canonical deny list into `~/.claude/settings.json`, syncs wiki scaffold + `.docs/guides/` scaffold, bootstraps Serena; does NOT set up deployment)
- `npx @codewizard-dt/bootstrap update` — runs `lib/scripts/update-project.sh` (installs hooks and skills globally, merges the canonical deny list into `~/.claude/settings.json`, syncs wiki scaffold + `.docs/guides/` scaffold; does NOT touch `.github/` workflows)
...
- `lib/scripts/setup-project.sh` — Set up a new project: installs MCPs and skills globally, syncs wiki scaffold + `.docs/guides/` scaffold, bootstraps Serena `project.yml`; deployment setup is explicit via `deploy`
...
- `lib/scripts/setup-deployment.sh` — Deployment/CI scaffolding. Copies `.github/` workflows + `.gitleaks.toml` into a project. Only called explicitly via `npx @codewizard-dt/bootstrap deploy`; never by `setup` or `update`. Copy-once: `security.yml` always overwritten (generic), `build.yml` + `.gitleaks.toml` skipped if present.
